...
- 1.0.04
	- Изменена первичная настройка GPIO. У датчика `ZG-102ZA HOBEIAN` включение светодиода происходит низким уровнем, из-за чего он все время промаргивает светодиодом, так, как настройка GPIO происходит не только при старте, но и каждый раз, когда чип выходит из состояния сна.
	- Изменен повтор команд `OnOff`. Сейчас датчик ждет ответа. Если ответ на команду пришел, то повторно команду не высылаем.
- 1.0.05
	- Изменен повтор команды `OnOff`. Добавлена проверка доставки команды через `aps ack`. Команда отправляется первый раз, если `aps ack` вернул ошибку, то команда отправляется повторно.
	- Добавлена повторная отложенная передача команды `OnOff`, если устройство не в сети и требуется `rejoin`. Проверка статуса сети выполняется 3 раза с паузой в 1 секунду. Если за это время датчик не присоединился к сети, то попытки отправить команду прекращаются.
